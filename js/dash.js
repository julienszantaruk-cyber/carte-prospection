...
export function initDash(onOpen){
  ...
  EL['dash-top']?.addEventListener('click', (e) => {
...
export function render(){
  ...
  /* ─── KPIs ─── */
  const scored = src.filter(p => p.score != null);
  ...
  EL['dash-kpis'].innerHTML = [
    ...
    kpi('Score moyen', avg, `${scored.length} lieux notés`),
    kpi('Partenaires', partners, n ? `${Math.round(partners/n*100)} %` : ''),
    kpi('Relances dues', late, late ? 'à traiter' : 'rien d\'urgent'),
    kpi('Favoris', favs),
    kpi('Sans coordonnées', noGeo, noGeo ? 'invisibles sur la carte' : 'tout est géolocalisé')
  ].join('');
  ...
  EL['dash-by-type'].innerHTML = bars('Par type',
  ...
  /* ─── Par statut ─── */
  EL['dash-by-status'].innerHTML = bars('Par statut',
    STATUSES.map(s => ({
      label: s.l,
  ...
  EL['dash-by-region'].innerHTML = bars('Par région',
    [...byReg.entries()]
